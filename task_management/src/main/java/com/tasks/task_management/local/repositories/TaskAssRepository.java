...
public interface TaskAssRepository extends JpaRepository<TaskAssignment, BigInteger> {
    ...
    @Transactional
    @Modifying
    void deleteAllByTask_TaskIdAndUserIdAndAssignedBy(BigInteger taskId,
                                                      BigInteger userId,
                                                      BigInteger assignedBy);

    boolean existsByTask_TaskIdAndUserId(BigInteger taskId, BigInteger userId);

    boolean existsByTask_TaskIdAndUserIdAndAssignedBy(BigInteger taskId,
                                                      BigInteger userId,
                                                      BigInteger assignedBy);

    @Transactional
    @Modifying
    void deleteByUserIdAndAssignedBy(BigInteger userId, BigInteger assignedBy);
